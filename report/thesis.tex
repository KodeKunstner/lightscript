...
The bottom-up approach could be like stepwise refinement \cite{stepwise-refinement}, more agile \cite{agile-manifesto}, starting out with a quick prototype, which gets more and more refined.
...
The hardware contains an 48MHz ARM cpu\cite{arm7tdmi}, which is the same as used in many mobile devices, 256KB of ROM, and 64KB of RAM, plus a coprocessor, a low resolution small display and a couple of buttons. \index{ARM cpu}
...
\label{spidermonkey}
\index{SpiderMonkey}
SpiderMonkey \cite{spidermonkey} is the JavaScript engine in Mozillas browsers, and it is also usable as an embeddable engine in other applications. %TODO:verify-that-I-remember-correctly-here.
...
Like Yolan, LightScript will run on CLDC 1.0, and will use standard classes for better embedding and code footprint size. It will support run time loading of source code, and will have functions as first class values. See the Yolan design choices on page~\pageref{yolandesign}.
...
By removing the rejecting part, and only requiring that, the can parsers build a syntax tree from valid programs, the parser may be optimised to run better on the limited devices.
...
LightScript only supports integers as number type, due to the platform. This is radically different from EcmaScript, which only supports floating point numbers. Script can still be compatible: addition, and multiplication, subtraction and modulo operations stay within the integer subset of floating point numbers, as long as there are no overflow, and they start out with integers. The shifts and bitwise operators temporarily cast to integers in EcmaScript, so they are not an issue. The only problem of the operators is division, and the solution here is to omit the \verb|/| binary operator from lightScript, and instead allow integer division to be implemented as a function \verb|div|, which can also be implemented in EcmaScript with a combination of division and rounding.
