...
Mobile devices ranges from low end phones, which, if they are programmable at all, only support limited Java Mobile Edition midlets, up to advanced smart phones with performance resource similar to older desktop computers.

When looking mobile devices, it should be noticed some survey \cite{smartphonesurvey} only focus smart phones, and thus the market shares does not include the more low end devices.
...
Surveys about mobile devices often focus on smart phones (high end mobile devices) and thus market shares in surveys, such as \cite{smartphonesurvey} does not include the more low end devices.
...
\subsubsection{Examples of systems}
The small microcontrollers may have less than a kilobyte of RAM, and some kilobytes of possibly flashable ROM for the executable code. Examples of this is the PIC??? \cite{picspec}, or the ATmega 8-bit microcontrollers \cite{atmegaspec}.
Larger systems include the z80 \cite{z80}. 
...
When we get to the 32-bit systems, ARM \cite{arm-architecture} is the most common architechture.
An potential prototyping platform to play with is the Lego Mindstorm \cite{lego}, which contains a reflashable system with an ARM7TDMI\cite{arm7tdmi} CPU, which is common in the low end mobile phones.
...
\subsection{Smartphones and PDAs}
\subsubsection{Mobile web applications}
...
\paragraph{V8}\cite{v8} is a JavaScript implementation made by Google, and released in 2008 in conjunction with the release of their browser ``Chrome''. 
...
Version 1.1 will add more functions and operators, but will not make major changes to the semantics.
