...
Størrelsen af det eksekverbare program er den mest begrænsende faktor for Java Micro Edition, og sprogene udviklet i dette speciale fylder mindre end eksisterende scriptingsprog.
Udførselshastigheden er på niveau med større scripting\-sprogs\-implementationer,
og betydeligt hurtigere end mange af de små scriptingsprog.
...
Sproget har dynamisk virtkefelt, Lisp-inspireret syntaks, og bliver fortolket ved at gennemløbe syn\-taks\-træet og evaluere de enkelte knuder.


LightScript er kompatibelt med en delmængde af JavaScript, hvilket gør sproget mere tilgængeligt for andre. 
Det har statisk virkefelt, understøtter ``closures'', undtagelser og objekt orienteret programmering.
...
%TODO:or-one-and-a-half-year-check-the-reference 
...
\chapter{Yolan}
\label{yolan}
%TODO: change math-mode to italics
\index{Yolan}
...
If variables are looked up at every access at runtime, this is likely to be bottleneck for performance.  %<- TODO: reformulate
...
\label{benchmark}
\index{Benchmarks}
%TODO: Julia: The chapter could have an introduction that gives an overview of the things you want to measure. Mention why the code footprint is important
% TODO: Julia: In the benchmarks section, it would be good to say what is important about each benchmark.  Just giving a reference is not good enough, since the reader will probably not be industrious enough to go look it up.
...
% TODO: Julia: If possible, it would be good to have a proper grammar of each language.  Reviewers tend to like that.  If you could make a semantics as well, but perhaps there is not time for that.
...
%TODO: julia-question: is there anything to say about FScriptME since it is small to
...
%TODO: why above results as they are
...
\paragraph{Recursion:} Highly recursive benchmark, similar to the recursive control-flow benchmark from \cite{sunspider, shootout}. This uses a lot of stack space. On some of the languages where it fails, only the first part of it was implemented %TODO: not clear what meant by first part
