...
class OldStdLib implements LightScriptFunction {
    ...
    public Object apply(Object[] args, int argpos, int argcount) throws LightScriptException {
        ...
        switch (id) {
            ...
            case STD_NEW_ITERATOR: {
                if (thisPtr instanceof Hashtable) {
                    OldStdLib result;
                    result = new OldStdLib(STD_ENUMERATION_ITERATOR);
                ...
                if (thisPtr instanceof Stack) {
                    OldStdLib result;
                    result = new OldStdLib(STD_INTEGER_ITERATOR);
...
//                return new Integer(Code.toInt(arg1));
    ...
    public static void register(LightScript ls) {
        ...
        OldStdLib defaultGetter = new OldStdLib(STD_DEFAULT_GETTER);
        ...
        for (int i = 0; i < names.length; i++) {
            ls.set(names[i], new OldStdLib(i));
        }

        objectPrototype.put("hasOwnProperty", new OldStdLib(STD_HAS_OWN_PROPERTY));
        objectPrototype.put("toString", new OldStdLib(STD_TO_STRING));
        Hashtable object = clone(objectPrototype);
        object.put("create", new OldStdLib(STD_CLONE));

        // Create members for array
        arrayPrototype.put("push", new OldStdLib(STD_ARRAY_PUSH));
        arrayPrototype.put("pop", new OldStdLib(STD_ARRAY_POP));
        arrayPrototype.put("join", new OldStdLib(STD_ARRAY_JOIN));
        ...
        math.put("random", new OldStdLib(STD_RANDOM));
        math.put("floor", new OldStdLib(STD_FLOOR));

        objectPrototype.put("constructor", new OldStdLib(STD_OBJECT_CONSTRUCTOR));
        arrayPrototype.put("constructor", new OldStdLib(STD_ARRAY_CONSTRUCTOR));
        array.put("concat", new OldStdLib(STD_ARRAY_CONCAT));
        arrayPrototype.put("sort", new OldStdLib(STD_ARRAY_SORT));
        arrayPrototype.put("slice", new OldStdLib(STD_ARRAY_SLICE));
        stringPrototype.put("slice", new OldStdLib(STD_STRING_SLICE));
        stringPrototype.put("charCodeAt", new OldStdLib(STD_STRING_CHARCODEAT));
        string.put("fromCharCode", new OldStdLib(STD_STRING_FROMCHARCODE));
        string.put("concat", new OldStdLib(STD_STRING_CONCAT));
